...
=== How TCP guarantee all bytes be transferred as expected?

TCP is a connection-oriented protocol, stream-oriented protocol, TCP also is a byte-oriented protocol, TCP keeps track of application data sent and received at *byte level*, which means all bytes be transferred as expected.

* TCP numbers all data bytes that are transmitted in a connection.  
* TCP offers full-duplex service, and numbering is independent in each direction.
* When TCP receives bytes of data form application, it stores them in the sending buffer and numbers them. The numbering does not start from 0, TCP generates a random number bewteen 0 and 2^32 for the numbering the 1st byte.

With the bytes be numbered, TCP use the *sequence number* and *acknowledgment number* to guarantee all bytes be transferred as expected.
...
=== How congestion control algorithm impact on throughput?

Firstly, the TCP congestion control terms as below.

image:img/tcp-congestion-introduction.png[]
...
|The receiver not Acknowledge sender per segments, as the example of above figure, after receiving first 2 segments containing sequence number 90 and 100, the receiver sends a segment with acknowledge number 110, this segment is called Cumulative Acknowledgement. 

|Congestion window(cwnd)
|Imposes a constraint on the rate at which a TCP sender can send traffic. The cwnd value is the amount of unacknowledged data at the sender.

|TCP Throughput
|

----
TCP Throughput ≈ (cwnd / RTT ) [bytes/second] 
----

|Multiplicative Decrease
|If the TCP sender receives three duplicate ACKs for the same segment, TCP interprets this event as packet loss due to congestion and reduces the congestion window `cwnd` by half. This congestion window reduction is known as multiplicative decrease. 

|Additive Increase
|After decreasing cwnd by half, and as long as no other packet loss is detected, TCP will slowly increase `cwnd` again by 1 MSS per RTT. This congestion control phase essentially produces an additive increase in the congestion window. 

|Congestion Control
|TCP congestion control is referred to as an Additive-Increase Multiplicative-Decrease (AIMD) form of congestion control. AIMD gives rise to the *sawtooth* behavior shown in above figure, which also illustrates the idea of TCP *probing* for bandwidth—TCP linearly increases its congestion window size (and hence its transmission rate) until a triple duplicate-ACK event occurs. It then decreases its congestion window size by a factor of two but then again begins increasing it linearly, probing to see if there is additional available bandwidth.

|===

[source, bash]
.*Commands used to check congestion control algorithm*
----
...
[source, bash]
.*Congestion control algorithm comparision bash*
----
#!/bin/bash  
...
#tc qdisc add dev ens33 root handle 1: netem loss 0.01%

tc qdisc change dev ens33 root handle 1: netem loss 0.01% delay 0ms
sysctl -w net.ipv4.tcp_congestion_control=cubic
for i in 1 2 3 ; do ttcp -t 10.1.20.204 ; done
sysctl -w net.ipv4.tcp_congestion_control=reno
for i in 1 2 3 ; do ttcp -t 10.1.20.204 ; done
sysctl -w net.ipv4.tcp_congestion_control=htcp
for i in 1 2 3 ; do ttcp -t 10.1.20.204 ; done

tc qdisc change dev ens33 root handle 1: netem loss 0.01% delay 10ms
sysctl -w net.ipv4.tcp_congestion_control=cubic
for i in 1 2 3 ; do ttcp -t 10.1.20.204 ; done
sysctl -w net.ipv4.tcp_congestion_control=reno
for i in 1 2 3 ; do ttcp -t 10.1.20.204 ; done
...
tc qdisc change dev ens33 root handle 1: netem loss 0.01% delay 20ms
sysctl -w net.ipv4.tcp_congestion_control=cubic
for i in 1 2 3 ; do ttcp -t 10.1.20.204 ; done
sysctl -w net.ipv4.tcp_congestion_control=reno
for i in 1 2 3 ; do ttcp -t 10.1.20.204 ; done
...
tc qdisc change dev ens33 root handle 1: netem loss 0.01% delay 30ms
sysctl -w net.ipv4.tcp_congestion_control=cubic
for i in 1 2 3 ; do ttcp -t 10.1.20.204 ; done
sysctl -w net.ipv4.tcp_congestion_control=reno
for i in 1 2 3 ; do ttcp -t 10.1.20.204 ; done
...
tc qdisc change dev ens33 root handle 1: netem loss 0.01% delay 40ms
sysctl -w net.ipv4.tcp_congestion_control=cubic
for i in 1 2 3 ; do ttcp -t 10.1.20.204 ; done
sysctl -w net.ipv4.tcp_congestion_control=reno
for i in 1 2 3 ; do ttcp -t 10.1.20.204 ; done
...
tc qdisc change dev ens33 root handle 1: netem loss 0.01% delay 50ms
sysctl -w net.ipv4.tcp_congestion_control=cubic
for i in 1 2 3 ; do ttcp -t 10.1.20.204 ; done
sysctl -w net.ipv4.tcp_congestion_control=reno
for i in 1 2 3 ; do ttcp -t 10.1.20.204 ; done
...
image:img/tcpnbspcongestion-contro.png[]
...
RTT,cubic,reno,htcp
0,66.67,66.22,67.37
10,21.02,18.67,19.79
20,18.18,15.57,17.47
30,16.62,13.08,14.37
40,12.76,9.71,11.94
50,9.48,7.47,9.10
----

* cubic has the better performance, especially in high latency environment.
* TCP Congestion Control is for maximum use the network bandwidth even in high latency environment
* The goal of congestion control is to determine how much capacity is available in the network, so that a source knows how many packets it can safely have in transit (inflight). 

=== How the application level outage affect the data transfer?

`ttcp` has a `-L` option that can simulate `recv` outage, this section will demonstrate the `recv` side's outage cause the data transfer significantly downgraded or high latency.

When you use Wireshark to analyze a packet capture, it performs analysis on TCP connections and is able to flag certain behaviors that can help understand TCP performance. Some of these correspond to specific TCP messages, while others are Wireshark highlighting a connection state. These flags include:
...
The application level outage most likely will geneated this kinds of flag if you use Wireshark to capture the packets.

Before the detailed depiction of these flags, fistly list the following terminology specific to TCP:

* *Receive Window* - also simply called the TCP window, this is advertised by a TCP receiver to its peer. It is the window that is visible in the capture, and may be different in either direction in a connection. The receive window is usually the size of the available receive buffer. TCP receivers can adjust the size of the receive window by changing the window value in ACKs they transmit. 
* *Send Window* - technically the congestion window, this is an internal value in a TCP sender which indicates the maximum amount of data it can send without receiving an acknowledgement. This is constantly adjusted by the TCP Congestion Control algorithm. This value cannot be observed in a capture; it can be inferred, if a sender stops sending data even though the recipient's receive window is not full, but the actual value is never sent on the network.
...
*TCP Window Full:* if a TCP sender transmits a packet which results in the number of unacknowledged bytes - or bytes in flight - being the same as the recipient's receive window, this is called filling the window, and Wireshark reports TCP Window Full. When you see TCP Window Full flags, it usually means that the sender is using the full capacity of the TCP flow, limited by the recipient's receive window. The network may have higher capacity.   
...
*TCP Retransmission:* When TCP sends a packet, it waits for an acknowledgment to confirm if the packets sent by the sender are received by the receiving party. If the acknowledgment is not received within the timeout interval (known as the retransmission timeout or RTO), the sender retransmits the packet. Retransmissions can also be forced when a TCP sender receives duplicate ACKs.

*TCP Duplicate ACK:* When a TCP receiver receives packets out of order, which it interprets as data loss, it sends an ACK indicating the expected sequence number. After two duplicate ACKs, a TCP sender begins retransmission. 

[source, bash]
.*On the recv side, open 3 terminals and run the following commands:*
----
ttcp -r -l 512 -L 
tcpdump -nni ens33 port 5001 -w ttcp-r.pcap
for i in {1..300} ; do netstat -nt | grep 5001 | awk '{print $2, $3}'; sleep 1 ; done
----

[source, bash]
.*One the trans side, open 3 terminals and run the following commands:*
----
tcpdump -nni ens33 host 10.1.10.205 -w ttcp-t.pcap
for i in {1..300} ; do netstat -nt | grep 5001 | awk '{print $2, $3}'; sleep 1 ; done
ttcp -n 10 -t 10.1.10.205
----

The raw data for collected ttcp-t send queue and ttcp-r recv queue as link:files/send-recv-q.csv[send-recv-q.csv], the reference linear diagram as below:

image:img/tcp-readwrite-buffer.png[]

Note that the maximum size of `ttcp-t` send buffer is *417560*, and the maximum size of `ttcp-r` receive buffer is *889396*, the tcp send buffer and receive buffer controlled by the following 2 parameters:
...
which the initial send buffer size in `ttcp-t` host is `16384 bytes`, the maximum send buffer size is `4194304`, the initial receive buffer size in `ttcp-r` host is `87380 bytes`, and the maximum receive buffer size is `6291456`.

*Download tcpdump pcap files:* 

* Download Link: https://pan.baidu.com/s/14_G3SrHgHwzujTOCscEgoQ 
* Download Code: 3258

*Wireshark TCP Window Full*

image:img/tcp-latency-window-full.png[]

The `ttcp-t` has send quite a lot of bytes to `ttcp-r`, but not receive the acknowledgement from `ttcp-r`, all unacknowledged bytes are in `ttcp-r` receive buffer, Due to *unacknowledged bytes size requals with ttcp-r's receive window*, so Wireshark reports *TCP Window Full*.

*Wireshark TCP Zero Window*

image:img/tcp-latency-zero-window.png[]

Due to `ttcp-r's` receive buffer begins to fill, the receive window begins to reduce, the receive window reduced to 0, and tell the `ttcp-t` to stop sending, so Wireshark reports *TCP ero Window* to hint this.

*Wireshark TCP Dup Ack*

image:img/tcp-latency-dup-ack.png[]

Wireshark reports *TCP Dup Ack* probably caused by `ttcp-r` receives packets out of order, which it interprets as data loss, it sends an ACK indicating the expected sequence number.

*Wireshark TCP Retransmission*

image:img/tcp-latency-retransimission.png[]  

This probably caused by `ttcp-t` sends a packet, it waits for an acknowledgment to confirm if the packets sent by the sender are received by the receiving party. If the acknowledgment is not received within the timeout interval (known as the retransmission timeout or RTO), the `ttcp-t` retransmits the packet.

*tcodump raw data:*

* link:files/ttcp-t-raw.csv[ttcp-t-raw.csv]
* link:files/ttcp-r-raw.csv[ttcp-r-raw.csv]

image:img/ttcp-r-receive-window-size.png[]

Note that the maximum window size is 64240, the smallest window size is 0, which trigger Zero Window a showed in above.




== L4 Proxy/LB Device & Software

=== Reset on timeout setting on BIG-IP fastl4 profie
...
=== MSS Overwrite on BIG-IP fastL4 profile
