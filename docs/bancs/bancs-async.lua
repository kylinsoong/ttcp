...
local function read_and_send_msg(sp_name, conn_obj, is_client, rs_name)
	...
	if is_client then
		...
		-- 5: create light thread for RS receiving
		err = send_msg(sp_name, msg, persist)
	else
...
when SERVER_DATA {
	...
	log("real server get sp name %s", sp_name)
	local rs_conn = TCP.server.get(sp_name, rs_name)
	if not rs_conn  then
		log("get rs_conn err by name %s", rs_name) 
		return
	end
	-- main loop
	while true do
		err = read_and_send_msg(sp_name, rs_conn, false, rs_name)
		if err then
			log("real server stream has err %s.", err)
			break
		end
	end
	rs_conn:close()
...
when CLIENT_DATA {
	...
	log("client get sp name %s", sp_name)
	-- main loop
	while true do
		err = read_and_send_msg(sp_name, client, true)
		if err then
			log("client stream has err %s.", err)
			break
		end
	end
}
